...
function inserir_produto($dados){
	$p 				= new Produtos();
	$p->where('cod', trim($dados['cod']));
	$p->where('id_usu', $dados['id_usu']);
	$p->get();
	
	if($p->exists()){
			return $p;
	}
	$p->cod			= trim($dados['cod']);
	$p->desc		= $dados['desc'];
	$p->comprimento	= $dados['comprimento'];
	$p->largura		= $dados['largura'];
	$p->peso		= $dados['peso'];
	$p->altura		= $dados['altura'];
	$p->diametro	= $dados['diametro'];
	$p->comprimento	= $dados['comprimento'];
	$p->id_usu		= $dados['id_usu'];
	$p->save();
	return $p;	
}
